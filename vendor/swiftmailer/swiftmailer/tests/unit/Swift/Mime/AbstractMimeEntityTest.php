...
abstract class Swift_Mime_AbstractMimeEntityTest extends \SwiftMailerTestCase {
    ...
    public function testGetHeadersReturnsHeaderSet() {
        ...
        $entity = $this->_createEntity($headers, $this->_createEncoder(), $this->_createCache()
        );
    ...
    public function testGetEncoderFromConstructor() {
        ...
        $entity = $this->_createEntity($this->_createHeaderSet(), $encoder, $this->_createCache()
        );
    ...
    public function testIdIsAutoGenerated() {
        $entity = $this->_createEntity($this->_createHeaderSet(), $this->_createEncoder(), $this->_createCache()
        );
    ...
    public function testBoundaryCanBeRetrieved() {
        ...
        $this->assertRegExp(
                '/^[a-zA-Z0-9\'\(\)\+_\-,\.\/:=\?\ ]{0,69}[a-zA-Z0-9\'\(\)\+_\-,\.\/:=\?]$/D', $entity->getBoundary()
        );
    }
    ...
    public function testChildrenOfLevelAttachmentAndLessCauseMultipartMixed() {
        for ($level = Swift_Mime_MimeEntity::LEVEL_MIXED; $level > Swift_Mime_MimeEntity::LEVEL_TOP; $level /= 2) {
            ...
            $entity = $this->_createEntity($this->_createHeaderSet(array(
                        'Content-Type' => $cType,)), $this->_createEncoder(), $this->_createCache()
            );
            $entity->setChildren(array($child));
        }
    }
    ...
    public function testChildrenOfLevelAlternativeAndLessCauseMultipartAlternative() {
        for ($level = Swift_Mime_MimeEntity::LEVEL_ALTERNATIVE; $level > Swift_Mime_MimeEntity::LEVEL_MIXED; $level /= 2) {
            $child = $this->_createChild($level);
            $cType = $this->_createHeader(
                    'Content-Type', 'text/plain', array(), false
            );
    ...
    public function testChildrenOfLevelRelatedAndLessCauseMultipartRelated() {
        for ($level = Swift_Mime_MimeEntity::LEVEL_RELATED; $level > Swift_Mime_MimeEntity::LEVEL_ALTERNATIVE; $level /= 2) {
            $child = $this->_createChild($level);
            $cType = $this->_createHeader(
                    'Content-Type', 'text/plain', array(), false
            );
    ...
    public function testChildrenAppearNestedInString() {
        ...
        $child1 = new MimeEntityFixture(Swift_Mime_MimeEntity::LEVEL_ALTERNATIVE, "Content-Type: text/plain\r\n" .
                "\r\n" .
                'foobar', 'text/plain'
        );

        $child2 = new MimeEntityFixture(Swift_Mime_MimeEntity::LEVEL_ALTERNATIVE, "Content-Type: text/html\r\n" .
                "\r\n" .
    ...
    public function testMixingLevelsIsHierarchical() {
        ...
        $part = $this->_createChild(Swift_Mime_MimeEntity::LEVEL_ALTERNATIVE, "Content-Type: text/plain\r\n" .
                "\r\n" .
                'foobar'
        );

        $attachment = $this->_createChild(Swift_Mime_MimeEntity::LEVEL_MIXED, "Content-Type: application/octet-stream\r\n" .
                "\r\n" .
                'data'
        );
    ...
    public function testOrderingTextBeforeHtml() {
        $htmlChild = new MimeEntityFixture(Swift_Mime_MimeEntity::LEVEL_ALTERNATIVE, "Content-Type: text/html\r\n" .
                "\r\n" .
                'HTML PART', 'text/html'
        );
        $textChild = new MimeEntityFixture(Swift_Mime_MimeEntity::LEVEL_ALTERNATIVE, "Content-Type: text/plain\r\n" .
                "\r\n" .
    ...
    public function testBodyIsReadFromCacheWhenUsingToByteStreamIfPresent() {
        ...
        $entity = $this->_createEntity($this->_createHeaderSet(), $this->_createEncoder(), $cache
        );
        $entity->setBody('foo');
