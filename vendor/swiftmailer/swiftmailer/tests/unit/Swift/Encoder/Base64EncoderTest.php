...
class Swift_Encoder_Base64EncoderTest extends \PHPUnit_Framework_TestCase {
    ...
    public function testInputOutputRatioIs3to4Bytes() {
        ...
        $this->assertEquals(
                'MTIz', $this->_encoder->encodeString('123'), '%s: 3 bytes of input should yield 4 bytes of output'
        );
        $this->assertEquals(
                'MTIzNDU2', $this->_encoder->encodeString('123456'), '%s: 6 bytes in input should yield 8 bytes of output'
        );
        $this->assertEquals(
                'MTIzNDU2Nzg5', $this->_encoder->encodeString('123456789'), '%s: 9 bytes in input should yield 12 bytes of output'
        );
    }
    ...
    public function testPadLength() {
        ...
        for ($i = 0; $i < 30; ++$i) {
            ...
            $this->assertRegExp(
                    '~^[a-zA-Z0-9/\+]{2}==$~', $this->_encoder->encodeString($input), '%s: A single byte should have 2 bytes of padding'
            );
        }
        ...
        for ($i = 0; $i < 30; ++$i) {
            ...
            $this->assertRegExp(
                    '~^[a-zA-Z0-9/\+]{3}=$~', $this->_encoder->encodeString($input), '%s: Two bytes should have 1 byte of padding'
            );
        }
        ...
        for ($i = 0; $i < 30; ++$i) {
            ...
            $this->assertRegExp(
                    '~^[a-zA-Z0-9/\+]{4}$~', $this->_encoder->encodeString($input), '%s: Three bytes should have no padding'
            );
        }
    }
    ...
    public function testMaximumLineLengthIs76Characters() {
        ...
        $this->assertEquals(
                $output, $this->_encoder->encodeString($input), '%s: Lines should be no more than 76 characters'
        );
    }
    ...
    public function testMaximumLineLengthCanBeSpecified() {
        ...
        $this->assertEquals(
                $output, $this->_encoder->encodeString($input, 0, 50), '%s: Lines should be no more than 100 characters'
        );
    }
    ...
    public function testFirstLineLengthCanBeDifferent() {
        ...
        $this->assertEquals(
                $output, $this->_encoder->encodeString($input, 19), '%s: First line offset is 19 so first line should be 57 chars long'
        );
    }
