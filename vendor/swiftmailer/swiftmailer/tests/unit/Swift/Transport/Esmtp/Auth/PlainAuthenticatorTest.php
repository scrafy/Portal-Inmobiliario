...
class Swift_Transport_Esmtp_Auth_PlainAuthenticatorTest extends \SwiftMailerTestCase {
    ...
    public function testSuccessfulAuthentication() {
        ...
        $this->assertTrue($plain->authenticate($this->_agent, 'jack', 'pass'), '%s: The buffer accepted all commands authentication should succeed'
        );
    }
    ...
    public function testAuthenticationFailureSendRsetAndReturnFalse() {
        ...
        $this->assertFalse($plain->authenticate($this->_agent, 'jack', 'pass'), '%s: Authentication fails, so RSET should be sent'
        );
    }
