...
class PersonTest extends \PHPUnit_Framework_TestCase {
    ...
    protected function isValidCnp($cnp) {
        if (
                is_string($cnp) && (bool) preg_match(static::TEST_CNP_REGEX, $cnp) && checkdate(substr($cnp, 3, 2), substr($cnp, 5, 2), substr($cnp, 1, 2))
        ) {
            ...
            if (
                    ($checksum < 10 && $checksum == substr($cnp, -1)) || ($checksum == 10 && substr($cnp, -1) == 1)
            ) {
                return true;
            }
        }
