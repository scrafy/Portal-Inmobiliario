...
class RotatingFileHandlerTest extends TestCase {
    ...
    private function assertErrorWasTriggered($code, $message) {
        if (empty($this->lastError)) {
            $this->fail(
                    sprintf(
                            'Failed asserting that error with code `%d` and message `%s` was triggered', $code, $message
                    )
            );
        }
    ...
    public function testAllowOnlyFixedDefinedDateFormats($dateFormat, $valid) {
        ...
        if (!$valid) {
            $this->assertErrorWasTriggered(
                    E_USER_DEPRECATED, 'Invalid date format - format must be one of RotatingFileHandler::FILE_PER_DAY ("Y-m-d"), ' .
    ...
    public function testDisallowFilenameFormatsWithoutDate($filenameFormat, $valid) {
        ...
        if (!$valid) {
            $this->assertErrorWasTriggered(
                    E_USER_DEPRECATED, 'Invalid filename format - format should contain at least `{date}`, because otherwise rotating is impossible.'
            );
        }
    }
